...
class NetworkBytes {
    ...
    func getBoolValue(bitIdx:Int) -> Bool {
        let byteIdx = bitIdx / 8
        let relBitIdx = bitIdx % 8
        let value = (UInt8(self[byteIdx]) << relBitIdx) >> 7
        return value == 1
    }
    ...
    func setBytes(values: [(Int, Int)], startByteIdx: Int = 0) {
        ...
        for (value, length) in values {
            ...
            if currentLength == 32 {
                ...
                let zeros = Data(repeating: 0x00, count: bytes.count - byteIdx + 1)
                bytes.append(zeros)
                
                withUnsafeBytes(of: &currentLine32) {
                    bytes.replaceSubrange(byteIdx..<byteIdx+4, with: $0)
                }
                byteIdx += 4
                currentLength = 0
                currentLine = 0
...
let bytes = NetworkBytes(data)
print(bytes.getIntValue(startBitIdx: 0, endBitIdx: 31))
